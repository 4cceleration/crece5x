...
- Cada pregunta tiene peso `w` (1–3). Puntaje de dimensión = Σ(w·s) / Σ(w aplicables) × 100.
...
- Grupo 1 si emite valores o es entidad de interés público, o (activos > 30.000 SMMLV o planta > 200 empleados).
...
2. **Chequeos determinísticos (código, sin IA)** — activo = pasivo + patrimonio (tolerancia 0.5%), utilidad del ERI coherente con variación de resultados en patrimonio, efectivo final del flujo = efectivo del ESF, presencia de los estados obligatorios según grupo, existencia de comparativo. Cada falla es un hallazgo con severidad fija.
...
**Proveedor agnóstico**: Vercel AI SDK con AI Gateway. El modelo se define en `AI_MODEL` (formato `proveedor/modelo`). Sin `AI_GATEWAY_API_KEY`, se usa un proveedor simulado que devuelve un resultado de ejemplo, para desarrollo y pruebas.
...
| 5 | Carga | Subida PDF/XLSX (máx. 10 MB) a almacenamiento privado vía interfaz `Storage` |
...
| 8 | Agenda | Disponibilidad semanal del consultor, reserva/cancelación, `.ics` |
...
| 13 | Administración | Preguntas y pesos, settings (umbrales, reglas), consultores, métricas simples |
...
- `question` (id, dimension, text, help, weight, requiresFlag nullable, order, active) · `answer` (consultationId, questionId, value ∈ {si, parcial, no, nose}).
- `upload` (id, consultationId, kind ∈ {esf, eri, flujo, patrimonio, notas, completo}, fileName, storageKey, mime, size) · `analysis` (id, consultationId, status ∈ {pendiente, procesando, listo, error}, extracted JSON, ratios JSON, error).
...
- `availability` (consultantId, weekday, startMinute, endMinute) · `appointment` (id, consultantId, companyId, consultationId, startsAt, endsAt, status ∈ {reservada, cancelada, realizada}, notes).
...
Paleta:
- Naranja (acción/acento): `#F26B1D`, hover `#D95A10`, fondo suave `#FFF4EC`.
...
Navegación empresa (3 ítems): **Inicio · Academia · Agenda**. Consultor: **Agenda · Casos**. Admin: **Preguntas · Ajustes · Usuarios**.
...
- `/consultor`, `/consultor/casos/[consultationId]`
- `/admin/preguntas`, `/admin/ajustes`, `/admin/usuarios`

## 7. Manejo de errores

- Archivo inválido o > 10 MB: mensaje en línea, sin subir.
...
- Vercel: variables `DATABASE_URL`, `DATABASE_AUTH_TOKEN`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `AI_GATEWAY_API_KEY`, `AI_MODEL`, `BLOB_READ_WRITE_TOKEN`, `RESEND_API_KEY`, `MAIL_FROM`. Migraciones contra Turso con `drizzle-kit migrate`.
